...
import (
	...
	interactionhttp "github.com/ajay/portfolio-backend/internal/interaction/transport/http"
)
...
func Registrar() bootstrap.Registrar {
	return func(cfg config.ServiceConfig, log *slog.Logger, r chi.Router) error {
		...
		interactionhttp.Register(r, svc)
		log.Info("interaction service wired", "addr", cfg.HTTP.Addr())
		return nil
	}
}
